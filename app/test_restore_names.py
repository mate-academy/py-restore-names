...
def test_restore_names_existing_names_not_changed():
    users = [
...
def test_restore_names_empty_first_name():
    users = [
        {"first_name": "", "last_name": "Taylor", "full_name": "Chris Taylor"},
    ]
    restore_names(users)
    assert users[0]["first_name"] == "Chris"

def test_restore_names_extra_spaces_in_full_name():
    users = [{"first_name": None, "last_name": "Brown", "full_name": "  Lucy   Brown  "} ]
    restore_names(users)
...
def test_restore_names_multiple_words_in_full_name():
    users = [{"first_name": None, "last_name": "Jr.", "full_name": "John Doe Jr."}]
    restore_names(users)
    assert users[0]["first_name"] == "John"

def test_restore_names_in_place_mutation():
    users = [{"first_name": None, "last_name": "Rogers", "full_name": "Steve Rogers"}]
    restore_names(users)
