...
def test_restore_only_missing_names(
    input_users: List[Dict[str, str]],
    expected_users: List[Dict[str, str]],
) -> None:
    restore_names(input_users)
    assert input_users == expected_users, (
        "Tests should check function with users "
        "whose first_name is missing"
    )
            ...
            [
                {
                    "first_name": None,
                    "last_name": "Green",
                    "full_name": "Lana Green",
                },
                {
                    "first_name": None,
                    "last_name": "Black",
                    "full_name": "Derek Black",
                },
            ],
            [
                {
...
def test_restore_only_none_names(
    ...
    assert input_users == expected_users, (
        "Tests should check function with users "
        "whose first_name is equal to None"
    )
