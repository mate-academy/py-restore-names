...
```
Помилка бази даних. Деякі користувачі втратили значення з поля `first_name`, але
`user` dict має поле `full_name`, тому ми можемо відновити `first_name`.

Напишіть тести для функції `restore_names`, яка приймає список `користувачів` і встановлює
виправити `first_name` для користувачів, які не мають його або дорівнює `None`. Ви
не повинна нічого повертати з функції.
