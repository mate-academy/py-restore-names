...
def test_restore_only_missing_names(monkeypatch):
    ...
    monkeypatch.setattr(restore_names,
                        "restore_names", restore_only_missing_names)

    test_result = pytest.main(["app/test_restore_names.py"])
    assert (
        test_result.value == 1
    ), "Tests should check function" \
       " with users whose first_name is equal to None"
...
def test_restore_only_none_names(monkeypatch):
    ...
    monkeypatch.setattr(restore_names,
                        "restore_names", restore_only_none_names)
