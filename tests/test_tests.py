...
def create_user(first_name: str | None, last_name: str, full_name: str) -> Dict[str, str]:
...
def test_restore_only_none_names(monkeypatch) -> None:
    def restore_only_none_names(users: List[Dict[str, str]]) -> None:
        for user in users:
            if user["first_name"] is None:
                user["first_name"] = user["full_name"].split()[0]

    monkeypatch.setattr('app.restore_names.restore_names', restore_only_none_names)
    ...
    assert actual_names == expected_names, "First names should be restored correctly from full names."
